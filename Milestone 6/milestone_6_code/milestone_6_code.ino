...
void solved(){
  ...
  buzzer.playFromProgramSpace(completed);
...
void path_reduce(){
  ...
  for(int path_location=1; path_location < turn_counter; path_location++){

    if(path[path_location] == 'U'){
      char a = path[path_location - 1];
      char b = path[path_location + 1];
      char c = path[path_location + 2];
    }
    if(a == 'L' && b == 'L') {
      path[a] = 'S'; // replace the first element of the three-item sequence
      
      for (int i = ; i < turn_counter; ++i) { // and then shift remaining elements
         path[i] = path[i + 1];
         }     
    }
    if(a == 'L' && b == 'S') {
